...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-----------------TRIGGERS----------------------------------------------------------
-- Actualizaci�n de monto de factura al agregar un detalle
...
BEGIN
    ...
    -- Obtener el token de federaci�n activo correspondiente al usuario y plataforma
...
BEGIN
    ...
    -- Verificar si el usuario est� activo y las credenciales son correctas
    SELECT TOP 1
...
BEGIN
    ...
    -- Obtener el id de usuario reci�n generado
...
BEGIN
    ...
    -- Verificar si se encontr� un usuario activo asociado al token
    ...
    ELSE
    BEGIN
        -- Devolver un valor negativo o un c�digo de error para indicar que no se encontr� un usuario activo
        SET @id_federacion = -1;
    END
END;
go
...
BEGIN
    ...
    -- Obtener el id de la federaci�n activa correspondiente al usuario y plataforma
    ...
    -- Actualizar la federaci�n encontrada
    ...
    BEGIN
		...
		if @@ROWCOUNT = 0
		begin
			raiserror('Error en la inserci�n. No se finaliz� la federaci�n', 16, 1)
		end
	...
	ELSE
	BEGIN
		RAISERROR('No se encontr� federaci�n activa asociada al usuario y plataforma indicados.', 16, 1);
	END
END;
go
...
BEGIN
    ...
    -- Seleccionar la lista de g�neros
...
BEGIN
	...
	if @@ROWCOUNT = 0
	begin
		raiserror('Error en la inserci�n. No se desuscribi� la plataforma', 16, 1)
	end
END;
go
...
BEGIN
    ...
    BEGIN TRY
		...
		-- Desactivar contenidos existentes que no est�n en la lista
        UPDATE c
        SET c.activo = 0
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Desactivar registros existentes que no est�n en la lista
        UPDATE px
        SET px.activo = 0
...
BEGIN
    ...
    -- Verificar que existe una federaci�n activa con la plataforma
    IF EXISTS (
        SELECT 1
        FROM dbo.Federaciones f
        WHERE f.id_usuario = @id_usuario
    ...
    BEGIN
		...
		if @@ROWCOUNT = 0
		begin
			raiserror('Error en la inserci�n. No se registr� la visualizaci�n', 16, 1)
		end
    END
    ELSE
    BEGIN
        -- Mensaje de error si no hay federaci�n activa
        RAISERROR ('Error: No hay federaci�n activa con la plataforma.', 16, 1);
    END;
END;
GO
...
begin
	...
	begin
		...
		if @@ROWCOUNT = 0
		begin
			raiserror('Error en la inserci�n. No se agreg� la plataforma', 16, 1)
		end
	end
end;
go
...
begin
	...
	if @@ROWCOUNT = 0
	begin
		raiserror('Error en la actualizaci�n. No se modific� la plataforma', 16, 1)
	end
end;
go
...
begin
	...
	if @@ROWCOUNT = 0
	begin
		raiserror('Error en la desactivaci�n. No se modific� la plataforma', 16, 1)
	end
...
begin
	...
	begin
		...
		begin
			...
			if @@ROWCOUNT = 0
			begin
				raiserror('Error en la modificaci�n. No se modific� el banner', 16, 1)
			end
		end
		else
		begin
...
begin
	...
	begin
		...
		if @@ROWCOUNT = 0
		begin
			raiserror('Error en la inserci�n de Publcista. No se registr� el usuario', 16, 1)
		end
		...
		if @@ROWCOUNT = 0
		begin
			raiserror('Error en la inserci�n de Publcista. No se registr� el token de usuario', 16, 1)
		end
		...
		if @@ROWCOUNT = 0
		begin
			raiserror('Error en la inserci�n de Publcista. No se registr� el publicista', 16, 1)
		end
	end
	else
	begin
...
begin
	...
	begin
		...
		if @@ROWCOUNT = 0
		begin
			raiserror('Error en la modificaci�n del Publcista. No se registraron los nuevos datos', 16, 1)
		end
	end
	else
	begin
...
begin
	...
	begin
		...
		if @@ROWCOUNT = 0
		begin
			raiserror('Error en la eliminaci�n del Publcista. No se registraron los cambios', 16, 1)
		end
...
begin
	...
	if @@ROWCOUNT = 0
	begin
		raiserror('Error en la actualizaci�n de datos', 16, 1)
	end
end
go
...
begin
	...
	if @id_usuario is null
	begin
		raiserror('El token proporcionado no corresponde a ning�n usuario', 16, 1)
	end
...
BEGIN
    ...
    -- Obtener el id_publicidad m�s bajo cuando no hay ninguna publicidad que cumpla la condici�n
    SELECT TOP 1 @min_id_publicidad = id_publicidad
    FROM dbo.Publicidades
    WHERE fecha_fin >= GETDATE()
    ORDER BY id_publicidad;

    -- Seleccionar el id_publicidad de las publicidades que cumplen la condici�n o el m�nimo si no hay ninguna
