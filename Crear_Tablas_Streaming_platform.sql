...
BEGIN
    ...
    -- Verificar si se encontr� el id de servicio
...
BEGIN
    ...
    -- Actualizar la fecha de uso de la sesi�n
    UPDATE dbo.Sesiones
...
BEGIN
	...
	SELECT @resultado = CASE
								WHEN fecha_uso IS NOT NULL THEN 2 -- Sesi�n usada
								WHEN fecha_vencimiento < GETDATE() THEN 3 -- Sesi�n vencida
								WHEN fecha_baja IS NOT NULL THEN 4 -- Sesi�n dada de baja
								WHEN id_sesion IS NOT NULL THEN 1 -- Sesi�n v�lida
...
BEGIN
    ...
    -- Insertar la sesi�n con fecha de vencimiento media hora despu�s de la creaci�n
...
BEGIN
    ...
    -- Obtener la informaci�n del login
    SELECT @valido = 1,
           @id_servicio = l.id_servicio
    FROM dbo.Logins l
    WHERE l.id_login = @id_login
          AND l.fecha_baja IS NULL  -- No est� dado de baja
          AND (l.fecha_expiracion IS NULL OR l.fecha_expiracion > GETDATE()) -- No est� expirado
...
BEGIN
    ...
    -- Validar el email y contrase�a en la tabla viewers
